...
if(isset($wpr_gensettings["wpr_mem4menus"]) && $wpr_gensettings["wpr_mem4menus"] == "1"){
...
function wpr_restrictPages($posts){
	...
	$nogo = true;
	/*ARYAN EDITS FOR UPDATE -- if settings not saved initially $wpr_membersettings["wpr_mem_redirurl"] is illegal offset*/
	$gohere = home_url();
	if(isset($wpr_membersettings["wpr_mem_redirurl"])){
	...
	if(!empty($posts)) {	
	  if(count($posts) == 1 && isset($posts[0]->post_type)){
		...
		foreach($obj as $trm){
			$chall = get_term_children($trm, 'wpr_membership');
			$obj = array_merge($obj, $chall);
		}
		...
		if(!$user_id && !headers_sent()){			
			...
			$allowed = get_excluded_taxs($user_id, "include");
			//$allowed = wp_get_object_terms($user_id, 'wpr_membership', array('type'=>'user', 'fields'=>'ids'));
			if(!empty($obj)){
				foreach($allowed as $slug){
					if(in_array($slug, $obj)){$nogo = false; break;}
				}
			}else{$nogo = false;}
...
function intercept_file_request($query_vars){
	...
	$proceed = apply_filters("customize_protected_file", 1, $query_vars, $safedir);
	if($proceed === 0){
...
function get_excluded_taxs($user_id, $exorin="exclude"){	
	...
	if(!empty($orderedu) && is_array($orderedu)){
		foreach($orderedu as $i=>$term){
			if(in_array($term->term_id, $userhasterms)){
				if(!in_array($term->term_id, $useable)){
					$useable[] = $term->term_id;
	...
	$exclude = array();
	foreach($useable as $trme){
		$pares = get_term_top_most_parent($trme, "wpr_membership");
		if(!empty($pares)){$useable = array_merge($useable, $pares);}
	}
	$useable = array_unique($useable);
