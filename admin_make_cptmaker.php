	...
	if($meta_data['postnm'] != "wpr_made_cpt_data" && $meta_data['id'] != "allcptmakerdat"){return;}
		...
		<h4>Please note that the title above is required and this page will not publish until one is provided.  The title is the CPT title that will be displayed in the custom meta box area. If you are only using this to add taxonomies to a pre-existing post type, a title will still be required but it will do nothing and display nowhere.</h4>
			...
			foreach($advopts as $ko => $aopt){
				...
				$optsetvalo = $mtet[$ko] == "0"?"checked":"";
				echo '<div class="cptmakereq"><div class="pmp">' .$aopt . '</div>
				<div class="pip"><input type="radio" name="wpr_made_cpt_data[' . $ko . ']" value="1" ' . $optsetval . ' /><span class="nin">Yes</span><input type="radio" name="wpr_made_cpt_data[' . $ko . ']" value="0" ' . $optsetvalo . ' /><span class="nin">No</span></div></div>';
			}
			?>
			...
			<input type="radio" class="taxhierarch" value="1" name="javhierch" checked /><span class="nin">Yes</span>
			<input type="radio" class="taxhierarch" value="0" name="javhierch" /><span class="nin">No</span>
			</div>
			</div>
			...
			foreach($advtax as $tko => $atopt){
				...
				$optsetvalo = $tax[$tko] == "0"?"checked":"";
				echo '<div class="cptmakereq"><div class="pmp">' .$atopt . '</div>
				<div class="pip"><input type="radio" name="wpr_made_cpt_data[taxonomies][' . $k . '][' . $tko . ']" value="1" ' . $optsetval . ' /><span class="nin">Yes</span><input type="radio" name="wpr_made_cpt_data[taxonomies][' . $k . '][' . $tko . ']" value="0" ' . $optsetvalo . ' /><span class="nin">No</span></div></div>';
			}
			?>
			</div>
			</div>
