...
if(isset($wpr_gensettings["wpr_want_usercats"]) && $wpr_gensettings["wpr_want_usercats"] == "1"){
	require_once('usercategory.php');
}
/* Check if the user wants restricted content/membership*/
if(isset($wpr_gensettings["wpr_want_membership"]) && $wpr_gensettings["wpr_want_membership"] == "1"){
	...
	'section1' => array(
		...
		'fields' => array(
			...
			array(
				'id' => 'wpr_mem4menus',
				'type' => 'check',//text,check,radio,textarea,select, separator, html
				'label' => 'Allow restricted Menu Items (only if membership is checked)',
				'description' => "This will create a menu item interface that will allow you to assign membership levels to menu items..",
				'default_value' => '1',
				'grouped' => 'one'
			),
			array(
...
$wprAdminAmplify = new GenerateSettingsPages($settings, 'Admin Amplify Settings', 'Admin Amplify', 'menu', '', 'Amplify Your Admin', 'wpr_admin_amplify', plugins_url() . '/wpr-admin-amplify/imgs/amplify16.png', 'manage_options');
...
$amppagec = array(
	array(
		...
		'general_single' => 'Page Cat',
		'general_plural' => 'Page Cats',
...
$ampcptc = array(
	array(
		'id'=> 'wpramplifycpttax',
		'general_single' => 'CPT Category',
		'general_plural' => 'CPT Categories',
		'column_id' => 'ampcptcat',
...
$ampmetac = array(
	array(
		...
		'general_single' => 'Meta Category',
		'general_plural' => 'Meta Categories',
...
$ampformc = array(
	array(
		...
		'general_single' => 'Form Category',
		'general_plural' => 'Form Categories',
...
function reorderAmplifySubs($menu_order){
	global $submenu;
	if(empty($submenu)){return $menu_order;}
...
$makepageMet = new GeneratePostMeta($wprPagemaker, "allpagemakerdat", "Page Maker", "wpr_made_page_data", 'wprmakepages');
...
$makepageMet = new GeneratePostMeta($wprmakeCPT, "allcptmakerdat", "CPT Maker", "wpr_made_cpt_data", 'wprmakecpts');
...
$makepageMet = new GeneratePostMeta($wprmakeMeta, "allmetamakerdat", "Meta Maker", "wpr_made_meta_data", 'wprmakemeta');
...
$makepageMet = new GeneratePostMeta($wprformmaker, "allformmakerdat", "Form Maker", "wpr_made_form_data", 'wprmakeforms');
...
function WPRmakeMagic(){
	...
	*/
	foreach($amplifymeta as $ampMeta){
		...
		$metAdAta = array();
		$meti = 0;
		if(!is_array($ampMeta["data"]["fields"])){
			$ampMeta["data"]["fields"] = array();
		}
...
if(isset($wpr_gensettings["wpr_want_emaillogin"]) && $wpr_gensettings["wpr_want_emaillogin"] == "1"){
